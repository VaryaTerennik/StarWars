  ...
  renderDetail() {
    ...
    Object.keys(this.homeWorldData).forEach((sKey) => {
      sTemplate = sTemplate.replace(
        "$" + `{homeworld.${sKey}}`,
        this.homeWorldData[sKey]
      );
    });
    sTemplate = sTemplate.replace(
      "${filmsList}",
      Film.renderDetailTableRow(this.filmsData)
    );
    sTemplate = sTemplate.replace(
      "${starshipsList}",
      Starship.renderDetailTableRow(this.starshipsData)
    );
    ...
    // console.log(newBack);
    document.querySelector(".actor-detail").style.background =
      "url(" + newBack + ") center no-repeat black";
  }
